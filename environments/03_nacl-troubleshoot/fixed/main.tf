...
locals {
  ...
  user_data = <<-EOT
    ...
    echo '<html><body><h1>NACL Troubleshoot Web Server</h1><p>Port: 8080 / Environment: fixed</p></body></html>' > /var/www/html/index.html
...
# -------------------------------------------------------
# Network ACL (FIXED)
# 修正: ルール200でエフェメラルポート(1024-65535)を追加
# → レスポンスパケットが正常に通過しクライアントに届く
# -------------------------------------------------------
